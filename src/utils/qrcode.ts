...
class QRCodeModel {
  ...
  makeImpl(test: boolean, maskPattern: number) {
    ...
    if (this.dataCache == null) {
      this.dataCache = QRCodeModel.prototype.createData(
        this.typeNumber,
...
export type QRModeModel = {
  [K: string]: number
}
const QRMode: QRModeModel = {
  MODE_NUMBER: 1 << 0,
  MODE_ALPHA_NUM: 1 << 1,
  MODE_8BIT_BYTE: 1 << 2,
  MODE_KANJI: 1 << 3,
}
const QRErrorCorrectLevel: any = { L: 1, M: 0, Q: 3, H: 2 }
const QRMaskPattern = {
  PATTERN000: 0,
  PATTERN001: 1,
  PATTERN010: 2,
...
class QRRSBlock {
  ...
  totalCount: number
  dataCount: number
  constructor(totalCount: number = 0, dataCount: number = 0) {
    this.totalCount = totalCount
    this.dataCount = dataCount
  }
  getRSBlocks(typeNumber: number, errorCorrectLevel: number) {
    var rsBlock = QRRSBlock.prototype.getRsBlockTable(
      typeNumber,
      errorCorrectLevel
    )
  ...
  getRsBlockTable(typeNumber: number, errorCorrectLevel: number) {
    let oQRRSBlock = new QRRSBlock()
    switch (errorCorrectLevel) {
      case QRErrorCorrectLevel.L:
        return oQRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 0]
      case QRErrorCorrectLevel.M:
        return oQRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 1]
      case QRErrorCorrectLevel.Q:
        return oQRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 2]
      case QRErrorCorrectLevel.H:
        return oQRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 3]
      default:
        return undefined
    }
  }
...
class QRBitBuffer {
  ...
  constructor() {}
  get(index: number) {
    var bufIndex: number = Math.floor(index / 8)
  ...
  putBit(bit: boolean) {
    let bufIndex: number = Math.floor(this.length / 8)
...
function _getAndroid(): boolean | number {
...
var svgDrawer = (function () {
  class Drawing {
    ...
    _htOption: any
    constructor(el: HTMLElement, htOption: any) {
      this._el = el
    ...
    draw(oQRCode: QRCodeModel) {
      ...
      function makeSVG(tag: string, attrs: any) {
    ...
    clear() {
      ...
      while (this._el.hasChildNodes())
        this._el.removeChild(this._el.lastChild as Node)
    }
  }
...
var Drawing = useSVG
  ...
  : (function () {
      ...
      class Drawing {
        ...
        _oContext: CanvasRenderingContext2D | null
        _elImage: HTMLImageElement
        _bIsPainted: boolean
        _android: number | boolean
        _htOption: any
...
export { QRErrorCorrectLevel, QRCode }
